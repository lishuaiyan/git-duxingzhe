...
public class RecordProcessor implements StatProcessor {
    ...
    private long currentRecordDate;

    //采样周期，单位 s
    private final int sampleInterval;
    private final String expectedExternalDeviceList;

    public RecordProcessor(int sampleInterval, int traceIdDiff, String expectedOperationName
    ...
    public void process(String[] recordParts) {
        ...
        if ("response".equals(recordType)) {
            ...
            if (reqTimeStamp - currentRecordDate < sampleInterval * 1000) {
                delayStatData = summaryResult.get(currentRecordDate);
            } else {
                currentRecordDate = reqTimeStamp;
                delayStatData = new DelayItem(currentRecordDate);
                delayStatData.getTotalDelay().addAndGet(delay);
                summaryResult.put(currentRecordDate, delayStatData);
            }
